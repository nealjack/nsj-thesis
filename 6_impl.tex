...
This is if we assume an income distribution similar to the EnHANTs Setup D and an income margin of 300\% of the expected workload power
\footnote{100\ssi{\micro\watt} represents a 300\% margin over a 24.5\ssi{\micro\watt} workload}.
...
This results in a 2.8\ssi{\milli\watt} average workload power\footnote{~$\frac{781\ssi{\milli\joule} + (197\ssi{\micro\watt} \times (300 - 40)\ssi{\second})}{300\ssi{\second}} = 2.77\ssi{\milli\watt}$}.

\placefigure{fig:impl:cam_sweep}
From this example workload and Camaroptera's income, we can use the heuristics developed in \cref{chap:intuition} to determine the proper capacity sizing for the platform.
...
\namec is capable of capturing 320x320 resolution color images, which can be directly fed to machine learning inference tools for object, person, or in this case: dog detection.
}}
\end{definefigure}
...
If we assume a periodic workload, capturing, compressing (at JPEG quality 90), and transmitting an image very ten minutes, \namec will require 148\ssi{\micro\watt} on average
\footnote{~$\frac{80.5\ssi{\milli\joule} + 14.5\ssi{\micro\watt}\times(600 - 7.97)\ssi{\second}}{600\ssi{\second}} = 148\ssi{\micro\watt}$}.
...
\begin{definefigure}{fig:ssim}
    \centering
    \includegraphics[width=\columnwidth]{figs/permacam/permacam_processing/jpeg_ssim.pdf}
...
\begin{definefigure}{fig:multiple}
    \centering
    \includegraphics[width=\columnwidth]{figs/permacam/permacam_processing/multiple_cameras.pdf}
...
\begin{definefigure}{fig:map}
    \centering
    \includegraphics[width=\columnwidth]{figs/permacam/permacam_processing/jpeg_mAP.pdf}
...
Compression Quality & Size (kB) & Latency (s) & Energy (mJ) \\
\hline
30  & 1.98 & 0.513 & 4.23\\
50  & 3.25 & 0.764 & 6.95\\
70  & 4.82 & 1.12 & 10.3\\
...
%We conclude our evaluation with a generalizable framework and metric, "cycles-per-byte". This framework helps ground decisions about when to process data locally, or send elsewhere for remote processing.
