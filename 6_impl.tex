...
The authors evaluated Camaroptera from 5--95\ssi{\kilo\lux}, which roughly corresponds to 4--77\ssi[per-mode=symbol]{\milli\watt\per\centi\meter\squared} 
\footnote{For natural sunlight: 1\ssi[per-mode=symbol]{\watt\per\meter\squared} $\approx$ 122\ssi{lux}~\cite{michael2020conversion}}.
We scale the mean of the Setup D trace to represent 10 and 50 \ssi{\milli\watt\per\centi\meter\squared} for an estimate of outdoor irradiance on the same scale.
...
\begin{definefigure}{fig:impl:camaroptera_pkt}
    \centering
    \includegraphics[width=\linewidth]{figs/chap6}
    \caption{
        ...
        Camaroptera operation is limited to times when daylight is available. The average number of packets is significantly lower during  
     }
\end{definefigure}
