...
With these relations, it is possible to calculate the energy contained in a battery of size $L^3$, the lifetime of that battery given an average workload power $P_w$, and the energy captured by a photovoltaic panel of size $L^2$ over the lifetime of the similarly sized battery.
...
\placefigure{fig:intuition:continuum}
...
To determine a minimum sufficient capacity, we are essentially sweeping two variables: capacity and average power.
...
\Cref{fig:intuition:required_capacity} shows the minimum capacity required to capture all of the available harvestable energy versus the magnitude of income and workload power. 
...
This result is a significantly more reasoned method for determining the minimum sufficient rechargeable energy capacity than other methods, like sizing capacity to support a single atomic operation in a workload, or sizing to support arbitrary amounts of operating time.
...
\subsection{The Impact of Income Safety Margin on Sufficient Capacity}
...
In \cref{fig:intuition:capacity_margin}, we model the impact of a income safety margin on the required capacity to capture sufficient energy. 
...
\Cref{fig:intuition:capacity_margin} presents required capacity for multiple sweeps of workload power under different safety margins. 
The different sweeps consider different safety margins of income power, from no margin to an 300\% margin. 
No margin indicates that the average workload power is equal to the income power, while an 300\% margin indicates the income power is 4$\times$ the amount required by the workload, on average\footnote{A safety margin is calculated as a percent difference, $100 \times \frac{\text{Income} - \text{Workload}}{\text{Income}}$}. 
...
With a 25\% margin, where the income provides 1.2$\times$ the average required workload power, a design that can expect an energy income with a similar distribution to the Setup D trace will require a capacity sized to \num{2.3E3}$\times$ the income power. 

\placefigure{fig:intuition:capacity_margin}
...
The end result of the analysis of this chapter is a reasoned heuristic for determining appropriate rechargeable capacity sizing for a system based on its income and workload that is superior to other arbitrary methods used by other platform designs. 
...
\begin{definefigure}{fig:intuition:continuum}
    \centering
