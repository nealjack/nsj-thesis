\chapter{Intuition for Wireless Sensor Power Supply Design}
...
The average energetic spectral irradiance, $\bar{E_e}$, is generally between 10 and 100\si{\micro\watt\per\centi\meter\squared} for photovoltaics in indoor conditions~\cite{yervaGrafting12,gorlatova2013networking}. 
...
A sensor workload can be represented as the time series of instantaneous powers over a time period $T$, as in \cref{eqn:intuition:work_sequence}. 
\begin{equation} \label{eqn:intuition:work_sequence}
    P_w = (w_{t})^{T}_{t=0} 
\end{equation}
The time series of required power for a system can be measured or it can be estimated if the sensor behavior is predictable and the power and duration of operations is known.
...
The irradiance for Setup B is very clearly almost all artificial light, as it consists of a pattern of five days with irradiance from artificial lighting followed by two days where humans are not rarely present during the weekend to turn the lights on. 
...
To isolate the impact of the variance of energy income, as well as the magnitude of the income, we can utilize the normalized traces to synthesize incomes with arbitrary average power.
...
At some point, a design can allocate enough capacity to consider its energy income as an average, even if it is highly variable.
...
These sequences for and energy income are defined by \cref{eqn:intuition:work_sequence,eqn:intuition:harvest_sequence}, respectively.
Given these traces of income and workload power, the trace of cumulative energy stored by an energy buffer is defined by \cref{eqn:intuition:iterative_capacity}. The initial condition $y_0$ assumes the buffer starts empty, which is realistic for capacitor-sized energy buffers, but a very conservative assumption for larger buffers. 
\begin{align}
\begin{split}
        y_0 &= \begin{cases} 
        0                       & \text{if $p_t - w_t < 0$} \\
...
For simplicity, the $\Delta t$ between each element of $P_h$ and $P_w$ can be assumed to be one second. This simplifies future calculations for energy.
...
Insufficient capacity will fill up prematurely, be unable to capture more energy, limiting the total energy captured.
The ideal capacity is one that allows enough energy capacity over the time period $T$ that represents greater than or equal average power compared to what is required by the workload. 
Given that the average income power is equal to the average workload power for this analysis, the ideal capacity is one that provides sufficient energy storage to capture all of the harvestable energy. 
The trace of energy captured by the buffer is represented by the sequence $Z = (z_t)^T_{t=0}$, and each element $z_t$ is defined by \cref{eqn:intuition:captured}.
...
Thus, we are essentially sweeping two variables: average power and capacity.
...
\begin{definetable*}{tab:intuition:enhants}
            ...
            Irradiance is expressed in units of \si[per-mode=symbol]{\micro\watt\per\centi\meter\squared.}
            }
\end{definetable*}
...
\begin{definefigure}{fig:intuition:traces}
    ...
    Setup A, B, and D measure irradiance in a students office. Setup A is located near a south facing window and receives some sunlight. Setup B is located at a bookshelf, and is largely occluded from sunlight. 
