...
The availability and magnitude of harvested power, the size of an energy buffer, and the intended sensor workload determine whether a design is minimally feasible, and if so, whether the design will be reliant on additional software or hardware intermittent techniques to ensure proper operation. 
...
infeasible, state retention required, and no state retention required.
Additionally, the figure illustrates in which conditions hysteresis management techniques are helpful or less helpful.
...
\end{equation}
In \cref{fig:intuition:framework} the blue region represents designs that satisfy \cref{eqn:intuition:checkpointing} and \textbf{Require State Retention} techniques to ensure proper operation.  
...
\begin{equation}
    ...
    E_{capacity} \geq \sum_{e \in E_a} e
\end{equation}
...
However, a system that satisfies \cref{eqn:intuition:no_checkpoint} may still experience intermittent operation depending on the variability of its energy income and size of its storage.
...
A design with average harvestable income less than what it takes to sustain a low power sleep state is better off turning off instead of sleeping. This constraint is defined by \cref{eqn:intuition:hysteresis}. 
\begin{equation}
    \label{eqn:intuition:hysteresis}
    P_{l} < P_{h,avg} \leq P_{s}
\end{equation}
Sensors within this regime will benefit from hysteresis techniques to reduce the amount of charge and time it takes for them to cold start.
Sensors that are able to harvest more power than it takes to maintain a deep sleep state will benefit from hysteresis management if they frequently must cold start. 
...
More energy and the capacity to hold it reduces the frequency of cold starts, and capacity has an averaging effect on energy income, providing power when income is insufficient, and storing in case of excess. 
With sufficient energy income and capacity, sensors can willfully enter low power sleep states for long periods, avoiding powering off and subsequent cold starting.
This point of sufficient capacity is denoted on \cref{fig:intuition:framework} by the
These sleep states can be managed by the sensor itself, instead of being imposed by the variability of energy income.
While it is relatively straightforward to determine component sizing to achieve proper energy income, as described in \cref{sec:intuition:energy_income}, it is more difficult to determine the proper energy capacity to avoid power loss and frequent cold starts.
...
For sensors that intermittenly sleep instead of powering off, deep
sleep power becomes analogous to leakage power, and hysteresis management techniques
