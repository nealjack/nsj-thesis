...
The proper capacity of a charge buffer for a given purpose exists on a continuum. 
...
For example, smaller capacitors are generally used for filtering out noise, slightly larger capacitors are used to stabilize and smooth out power supply ripple, and even larger capacitors can be used to provide instantaneous spikes in power when needed. 
...
Like energy income or the size of preallocated energy, the capacity of an energy buffer must be considered in a similarily principled manner.
...
Energy harvesting applications that can expect an amount of natural light should size their rechargeable capacity according to either the Setup C or D lines. Such an application would have to scale its capacity by at least 8--\num{9e6} times the expected magnitude of the income and workload power.
For indoor applications without significant seasonal variance, capacity can be sized according to the Setup A and B lines, to be at least 1--\num{4e6} times the magnitude of income and workload power.
...
\subsection{The Impact of Workload Margin on Sufficient Capacity}
...
\Cref{fig:intuition:capacity_margin} presents multiple sweeps of income power and the minimum sufficient capacity for that income. 
