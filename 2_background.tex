...
For example, the TI BQ25505 boost charger~\cite{bq25505} is capable of operating at \textgreater90\% efficiency at voltages higher than 1.5\ssi{\volt}.
...
To perform this task, based on technology available when this work was published, the burrow sensors required on average ~300\ssi{\micro\watt}. This power requirement is low enough to be powered by various energy harvesting methods, however there are not many options for harvesting within a sea bird burrow. Solar and thermal differential harvesting are not ideal. The inclusion of these harvesting methods would potentially make the burrows unappealing to sea birds, not to mention substantially increasing the deployment effort and cost.
...
Conservatively, ZebraNet collars require an average of 405\ssi{\milli\watt} to perform GPS sampling and occasional peer-to-peer and base station networking. ZebraNet collars are relatively high power, even compared to its contemporaries like the Great Duck Island burrow sensors.
GPS and long range radio technology were fairly power hungry at the time the collars were designed.
The authors note that to power a collar for just 5 days would require a 13.5 \ssi{\ampere\hour} non-rechargeable battery, weighing 4 pounds. Not only would a non-rechargeable battery be unable to provide sufficient energy, it would also be too large and heavy to put on a zebra's neck.
...
When online and transmitting, a BLE advertiser like the Williot pixel will require on the order of 10s of \ssi{\micro\ampere} when advertising a full payload every 500\ssi{\milli\second}~\cite{nrf52Power}. A tag that utilizes backscatter will require much less to transmit data, on the order of single \ssi{\micro\ampere} or less depending on sleep state efficiency~\cite{jeevawireless}. This average power is well below the capabilities for directed RF harvesting from a device similar to an RFID reader, which can typically supply hundreds of \ssi{\micro\watt} at a minimum~\cite{sample2008design}.
...
At this rate, CapBand requires 260\ssi{\micro\watt}. This does not include the energy required to transmit the measurements.
...
The FaceBit lifetime and workload corresponds to an average power of 286\ssi{\micro\watt}.
...
In worst-case indoor conditions, where dim artificial lighting only provides on the order of 10\ssi{\micro\watt\per\centi\meter\squared}~\cite{yervaGrafting12}
...
The 500\ssi{\micro\watt} storage capacitance and circuitry is small and amortized in comparison.
...
The first deployment utilized a Libelium Waspmote and an XBee 868LP sub-\ssi{\giga\hertz} radio~\cite{waspmote,xbeeradio}.
...
This configuration of 6 C-type batteries provides a maximum of 16\ssi{\ampere\hour} at 4.5V.
...
Despite the high energy density of primary lithium batteries, at the scale of a \ssi{\milli\meter}, the energy provided is insufficient to offer an adequate lifetime for these systems~\cite{lee2013modular}.
...
This represents an order of magnitude improvement over similar offerings, like the Nordic nRF5340, which requires 56\ssi[per-mode=symbol]{\micro\ampere\per\mega\hertz}~\cite{nrf5340}.
